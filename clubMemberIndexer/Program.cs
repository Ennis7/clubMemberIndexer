namespace clubMemberIndexer;
class Program
{
    public const int Size = 15;  // global variable
    class ClubMembers
    {
        private string[] Members = new string[Size];
        public string ClubType { get; set; }
        public string ClubLocation { get; set; }
        public string MeetingDate { get; set; }

        // constructor
        public ClubMembers()
        {
            for (int i = 0; i < Size; i++)
            {
                Members[i] = string.Empty;
            }
            ClubType = string.Empty;
            ClubLocation = string.Empty;
            MeetingDate = string.Empty;
        }
        //indexer get and set methods
        public string this[int index]
        {
            get
            {
                string tmp;
                if (index >= 0 && index <= Size - 1)
                {
                    tmp = Members[index];
                }
                else
                {
                    tmp = "";
                }
                return (tmp);
            }
            set
            {
                if (index >= 0 && index <= Size - 1)
                {
                    Members[index] = value;
                }
            }
        }
    }

    static void Main(string[] args)
    {
        string[] names = { "Jimmy John", "Joe Dirt", "James Donald", "Ben Jerry" };

        ClubMembers Club = new ClubMembers();
        for (int i = 0; i < names.Length; i++) {
            Club[i] = names[i];
        }
        Club.ClubLocation = "Public Library";
        Club.ClubType = "Chess";
        Club.MeetingDate = "Mondays at 7pm";

        Console.WriteLine($"Here is the club information.");
        Console.WriteLine($"Club Members:");

        for (int i = 0; i < Size; i++)
        {
            if (Club[i] != string.Empty)
                Console.WriteLine(Club[i]);
        }
        Console.WriteLine();
        Console.WriteLine($"Club Type: {Club.ClubType}");
        Console.WriteLine($"Club Location: {Club.ClubLocation}");
        Console.WriteLine($"Club Meeting: {Club.MeetingDate}");
        Console.ReadKey();



        //--------User input-------
        //
        //bool end = false;
        //while (!end)
        //{
        //    int sub = 0;
        //    while (sub < 1 || sub > Size)
        //    {
        //        Console.WriteLine($"How many members would you like to enter (enter 1-{Size})?");
        //        while (!int.TryParse(Console.ReadLine(), out sub))
        //            Console.WriteLine($"Enter a value between 1 - {Size}");
        //    }
        //    Console.WriteLine("Enter the name of the member");
        //    Club[sub - 1] = Console.ReadLine();
        //    Console.WriteLine("Press any key to continue, q to quit");
        //    string stop = Console.ReadLine();
        //    if (stop == "q")
        //        end = true;
        //}
        //Console.WriteLine("What club are they joining?");
        //Club.ClubType = Console.ReadLine();

        //Console.WriteLine("Where is the club located?");
        //Club.ClubLocation = Console.ReadLine();

        //Console.WriteLine("What time is the club meeting?");
        //Club.MeetingDate = Console.ReadLine();


        //Console.WriteLine($"Here is the club information.");
        //Console.WriteLine($"Club Members");
        //for (int i = 0; i < Size; i++)
        //{
        //    if (Club[i] != string.Empty)
        //        Console.WriteLine(Club[i]);
        //}
        //Console.WriteLine($"Club Type: {Club.ClubType}");
        //Console.WriteLine($"Club Location: {Club.ClubLocation}");
        //Console.WriteLine($"Club Meeting: {Club.MeetingDate}");
    }
}

